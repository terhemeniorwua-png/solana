...
import { HubHeroBackground } from "./HubHeroBackground";
...
function LinkAnchor({ link, className, children, ariaLabel }) {
  const external = link.external ?? isExternalHref(link.href);

  if (!external) {
    return (
      <NextLink href={link.href} className={className} aria-label={ariaLabel}>
        {children}
      </NextLink>
    );
  }
...
const PRIMARY_BUTTON = cn(
  BUTTON_BASE,
  "rounded-full h-11 px-6 bg-[#7b47ff] text-zinc-50 hover:bg-[#7b47ff]/90",
...
const SECONDARY_BUTTON = cn(
  BUTTON_BASE,
  "rounded-full h-11 px-6 border-[1px] border-nd-border-prominent not-hover:bg-transparent hover:bg-nd-border-prominent",
  "h-12 text-base md:text-lg no-underline text-inherit",
);

export function NavigationHubPage({ config }) {
  const accentColor = config.accentColor ?? "#14F195";
  const style = { "--hub-accent": accentColor };
          ...
          <div className="w-full flex flex-col xl:flex-row xl:pb-10 pb-[40px] md:pb-[64px] xl:pb-[80px]">
            ...
            {config.feature && (
              <HubFeatureCard
                ...
                hasMetrics={Boolean(config.metrics && config.metrics.length > 0)}
              />
            )}
          </div>
        )}
...
function HubMetrics({ metrics, hasFeature }) {
...
function HubFeatureCard({ feature, hasMetrics }) {
...
function HubOverviewSection({ overview, heroImageSrc }) {
      ...
      >
        ...
        <div className="mt-[48px] xl:mt-[88px] grid grid-cols-1 md:grid-cols-3 gap-px bg-white/10 border-t border-b border-white/10">
          {overview.points.map((point, idx) => (
            <div
              key={point.title}
              className={cn("bg-black p-6 md:p-8", idx === 0 && "md:border-r-0")}
            >
...
function HubPathways({ pathways, eyebrow, heading }) {
  return (
...
function HubDirectory({ sections, eyebrow, heading, description, primaryCta, secondaryCta }) {
  return (
    <section className="relative bg-black text-white text-left overflow-hidden">
      <div className={cn(HUB_FRAME, SECTION_PAD)}>
...
function HubButton({ link, variant }) {
  const external = link.external ?? isExternalHref(link.href);
    ...
    return (
      ...
      >
        <span className="-ml-2 p-1 !size-6 md:!size-8 bg-black text-white rounded-full inline-flex items-center justify-center shrink-0">
          {external ? (
            <ArrowOutUpRightSquare aria-hidden className="!size-[14px] md:!size-[16px] block" />
          ) : (
            <ArrowRight aria-hidden className="!size-[14px] md:!size-[16px] block" />
          )}
        </span>
  ...
  return (
    <LinkAnchor link={link} className={SECONDARY_BUTTON}>
      {link.title}
      {external ? (
