...
import { WALLET_CATEGORIES, WALLET_FEATURES, WALLET_PLATFORMS, getFeaturedEverydayWallets } from "@/data/walletTaxonomy";
import { buildDirectorySearchParams, DEFAULT_DIRECTORY_STATE, parseDirectoryState } from "@/data/walletsState";
import { walletsCopy } from "@/data/walletCopy";
import { buildT } from "@/lib/i18n";
import styles from "./WalletDirectory.module.scss";
import { WalletHeroScene } from "./WalletHeroScene";

const FEATURE_GROUPS = [
  { id: "ownership", options: ["non_custodial", "custodial", "mpc", "social_recovery"] },
  { id: "everyday", options: ["buy_crypto", "sell_crypto", "get_paid", "card_spending", "staking", "hold_nfts"] },
  { id: "solana", options: ["te", "blinks_and_actions", "solana_pay"] },
  { id: "security", options: ["hardware", "multi_sig", "spending_limits", "open_source"] },
  { id: "builders", options: ["gas_abstraction", "private_key_infrastructure"] },
  { id: "networks", options: ["multi_chain"] },
];

const CATEGORY_ORDER = ["consumer", "hardware", "institutional", "payments", "infrastructure"];

const PLATFORM_GROUPS = [
  { id: "apps", options: ["ios", "android", "desktop", "web", "hardware"] },
  { id: "extensions", options: ["chrome", "firefox", "brave", "edge"] },
  { id: "developers", options: ["api", "sdk"] },
];

const LEARN_RESOURCES = [
  { id: "whatIsWallet", href: "/learn/what-is-a-wallet" },
  { id: "sendingSol", href: "/learn/sending-and-receiving-sol" },
  { id: "stayingSafe", href: "/learn/staying-safe-on-solana" },
  { id: "staking", href: "/learn/what-is-staking" },
];
...
const directoryStore = {
  ...
  cachedState: null,
  subscribe(listener) {
    directoryStore.listeners.add(listener);
    return () => directoryStore.listeners.delete(listener);
  },
  emit() {
    directoryStore.listeners.forEach((listener) => listener());
  },
  getSnapshot() {
    if (typeof window === "undefined") return DEFAULT_DIRECTORY_STATE;
    const url = window.location.href;
    if (url !== directoryStore.cachedUrl) {
      directoryStore.cachedUrl = url;
      directoryStore.cachedState = parseDirectoryState(
        new URLSearchParams(window.location.search),
      );
    }
    return directoryStore.cachedState;
  },
  getServerSnapshot() {
...
function featuredWalletsOnce(wallets) {
  if (featuredWalletsOnce.cached && featuredWalletsOnce.key === wallets) {
    return featuredWalletsOnce.cached;
  }
  featuredWalletsOnce.key = wallets;
  featuredWalletsOnce.cached = getFeaturedEverydayWallets(wallets, {
    randomize: true,
  });
  return featuredWalletsOnce.cached;
}
...
function getWalletCategories(wallet) {
  return wallet.categories.length ? wallet.categories : [wallet.category];
}

function getWalletCategoryLabel(wallet, categoryLabels) {
...
function getFeatureMatchGroup(feature) {
  return feature === "non_custodial" || feature === "custodial"
    ? "custody_model"
    : feature;
}

function walletMatchesSelectedFeatures(wallet, selectedFeatures, ignoredGroup) {
...
function walletMatchesState(wallet, state, labels, options = {}) {
  ...
  if (!walletMatchesSelectedFeatures(wallet, state.features, ignoreFeatureGroup)) {
    return false;
  }
  ...
  if (ignoreGroup !== "search" && state.search.trim()) {
    ...
    const searchable = [
      wallet.name,
      wallet.description,
      ...getWalletCategories(wallet).map((category) => labels.categories[category]),
...
function WalletLogo({ wallet, size = "default" }) {
  ...
  if (!iconUrl) {
    return (
      <span className={`${styles.logoFallback} ${styles[`logoFallback-${size}`]}`}>
...
function WalletTags({ wallet, limit = 3 }) {
...
function PlatformList({ wallet }) {
  const platforms = wallet.platforms.slice(0, 5);
  const remaining = wallet.platforms.length - platforms.length;

  if (!platforms.length) {
    return <span className={styles.mutedText}>{t("card.platformPending")}</span>;
  }

  return (
    <p className={styles.platformLine}>
      {platforms.map((platform) => t(`taxonomy.platforms.${platform}`)).join(" · ")}
      {remaining > 0 ? ` +${remaining}` : ""}
    </p>
  );
}

function WalletCard({ wallet, categoryLabels }) {
  return (
...
function WalletRow({ wallet, categoryLabels }) {
  return (
...
export function WalletDirectory({ data, quickFeatureFilters }) {
  const state = useSyncExternalStore(
    directoryStore.subscribe,
    directoryStore.getSnapshot,
    directoryStore.getServerSnapshot,
  );
  const featuredWallets = useSyncExternalStore(
    () => () => {},
    () => featuredWalletsOnce(data.wallets),
  ...
  const commitState = (nextState, mode = "push") => {
  ...
  const updateState = (updater, mode) => {
  ...
  return (
    <main className={styles.page}>
        ...
        <header className={styles.hero}>
          ...
          <h1>
            {t.rich("hero.headline", {
              strong: (chunks) => (
                <strong className={styles.heroHeadlineEmphasis}>{chunks}</strong>
              ),
            })}
          </h1>
      ...
      <section className={styles.featuredStrip} aria-label={t("featured.ariaLabel")}>
      ...
      <section id="wallet-directory" className={styles.directorySection}>
        <div className={styles.directoryHeader}>
          ...
          <div className={styles.scopeControl}>
            ...
            <div className={styles.scopeSwitch} role="group" aria-label={t("directory.scope.label")}>
              <button
                type="button"
        ...
        <div className={styles.audienceSection}>
          ...
          <p className={styles.audienceSelectionDescription} aria-live="polite">
            ...
            {state.category === "all"
              ...
              : t(`directory.audience.categories.${state.category}.description`)}
          </p>
        </div>
        ...
        <div className={styles.toolbar}>
          ...
          <div className={styles.viewToggle} aria-label={t("directory.view.ariaLabel")}>
            <button
              type="button"
          ...
          {data.lastReviewed && (
            <p className={styles.lastReviewed}>
              ...
              <time dateTime={data.lastReviewed}>
                {formatVerificationDate(data.lastReviewed)}
              </time>
            </p>
          )}
        </div>
